...
void Simulation::MainSimulation() 
{
	...
	Color color = WHITE;
	//find a way for the detection of collision to recognize if it already detect the same object, 
	do {
		...
		if (temptr == NULL) {
			...
			for (auto& items : ObstaclesStorage) {
					...
					if (collisionChecker.DetectCollision(items, temptr)) {
						...
						std::cout << "\nHit\n";
						PoI = collisionChecker.PointOfIntersection(items.Obstacle_P1, items.Obstacle_P2, collisionChecker.t, collisionChecker.t);
						...
						if (currhit_id != items.Obstacle_id) {
